...
#include "webserver/mime_types.h"
...
void StaticFileController::serveStaticFile(const HttpRequest &req, HttpResponse &resp) {
    ...
    if (!ec) {
        auto lastModifiedTime =
                std::chrono::system_clock::to_time_t(std::chrono::clock_cast<std::chrono::system_clock>(lastModified));
        char timeBuf[100];
        std::strftime(timeBuf, sizeof(timeBuf), "%a, %d %b %Y %H:%M:%S GMT", std::gmtime(&lastModifiedTime));
...
std::time_t StaticFileController::parseHttpDate(const std::string &date) {
    std::tm tm = {};
...
std::string StaticFileController::generateETag(const std::filesystem::file_time_type &mtime, std::uintmax_t size) {
...
void StaticFileController::handleRangeRequest(const std::string &rangeHeader, std::uintmax_t fileSize,
                                              const std::string &filepath, const HttpRequest &req, HttpResponse &resp) {
    // 解析范围请求头
    std::smatch matches;
