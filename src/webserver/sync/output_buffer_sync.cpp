...
void OutputBufferSync::set_response_with_mmap(const char *response_data, size_t response_len, const std::string &file_path,
    ...
    if (!file_path.empty() && file_size > 0) {
        ...
        void *addr = mmap(nullptr, // 建议映射起始位置
                          map_size, // 映射字节数
                          PROT_READ, // PROT_WRITE | PROT_EXEC（加载可执行代码（如 .so 库）） | PROT_NONE
                          MAP_PRIVATE, // MAP_PRIVATE 私有写时复制，修改不影响原文件 | MAP_SHARED
                                       // 共享映射，修改会同步到文件，用于进程间通信、修改文件 | MAP_ANONYMOUS
                          fd,
        ...
        if (addr == MAP_FAILED) {
            LOG_ERROR("mmap failed: {} (offset={}, aligned={}, map_size={})", strerror(errno), file_offset,
...
void OutputBufferSync::set_response_with_sendfile(const char *response_data, size_t response_len,
...
void OutputBufferSync::write_to(int fd) {
    if (bytes_to_send_ == 0) {
        return;
    }
    ...
    if (use_sendfile_) {
        ...
        if (bytes_have_sent_ < response_bound_) {
            ...
            ssize_t n = fiber::IO::write(fd, base + bytes_have_sent_, header_remain).value_or(-1);

            if (n < 0) {
                if (errno == EAGAIN || errno == EWOULDBLOCK) {
                    throw std::runtime_error("Sync write returned early");
                }
        ...
        if (file_fd_ >= 0 && file_remain_ > 0) {
            ssize_t n = fiber::IO::sendfile(fd, file_fd_, &file_offset_, file_remain_).value_or(-1);

            if (n < 0) {
                if (errno == EAGAIN || errno == EWOULDBLOCK) {
                    throw std::runtime_error("Sync sendfile returned early");
                }
                ...
                throw std::runtime_error(std::string {"sendfile error "} + strerror(errno));
            }
        ...
        // mmap + writev 模式
        // sync send
        ssize_t n = fiber::IO::writev(fd, iov_, iov_count_).value_or(-1);
        if (n < 0) {
            if (errno == EAGAIN || errno == EWOULDBLOCK) {
                throw std::runtime_error("Sync writev returned early");
            }
            // 客户端主动断开（正常情况，如视频播放器获取足够数据后断开）
            if (errno == EPIPE || errno == ECONNRESET) {
            ...
            throw std::runtime_error(std::string {"writev error "} + strerror(errno));
        }
...
void OutputBufferSync::close_file_if_needed() {
    if (file_fd_ >= 0) {
        close(file_fd_);
        file_fd_ = -1;
    }
...
// 移动语义支持
OutputBufferSync::OutputBufferSync(OutputBufferSync &&other) noexcept :
    iov_{other.iov_[0], other.iov_[1]}, iov_count_(other.iov_count_), bytes_have_sent_(other.bytes_have_sent_),
    bytes_to_send_(other.bytes_to_send_), response_bound_(other.response_bound_), file_address_(other.file_address_),
    mmap_size_(other.mmap_size_), should_unmap_(other.should_unmap_) {
    other.reset();
...
OutputBufferSync &OutputBufferSync::operator=(OutputBufferSync &&other) noexcept {
    if (this != &other) {
        unmap_if_needed();
        iov_[0] = other.iov_[0];
        iov_[1] = other.iov_[1];
