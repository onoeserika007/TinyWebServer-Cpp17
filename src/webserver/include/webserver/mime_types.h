...
namespace MimeTypes {
    ...
    // 检查文件类型是否支持
    inline bool isSupportedType(std::string_view path) { return getMimeType(path) != "application/octet-stream"; }

    // 检查是否是文本类型
    inline bool isTextType(std::string_view mimeType) {
        return mimeType.starts_with("text/") || mimeType == "application/javascript" ||
