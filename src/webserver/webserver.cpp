...
EpollServer::EpollServer(const std::string &host, int port) : host_(host), port_(port) {
    ...
    // configs
    use_reactor_ = config_manager.get<bool>("server.use_reactor", true);
...
void EpollServer::handleRead(int fd) {
    ...
    if (use_reactor_) {
        ...
        thread_pool_.pushTask([this, fd = fd, timer]() {
            if (connections_[fd]->ReadOnce()) {
                if (timer) {
                    timer_manager_.refresh(timer);
                }
