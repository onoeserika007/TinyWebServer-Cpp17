...
bool MySQLConnection::connect(const std::string &host, const std::string &user, const std::string &pwd,
                              const std::string &db_name, unsigned int port) {
    MYSQL *ret = mysql_real_connect(conn_, host.c_str(), user.c_str(), pwd.c_str(), db_name.c_str(), port, nullptr, 0);
    return ret != nullptr;
}
...
MYSQL_ROW MySQLConnection::fetchRow() { return result_ ? mysql_fetch_row(result_) : nullptr; }
